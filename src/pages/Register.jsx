...
const copy = {
  ar: {
    title: "إنشاء حساب", emailTitle: "تأكيد البريد", phoneTitle: "تأكيد رقم الجوال", intro: "بيانات صحيحة لوصول أسرع وأكثر أماناً",
    identityNotice: "اكتب اسمك كما يظهر في الهوية الوطنية أو الإقامة أو جواز السفر. يمكنك تعديله قبل مراجعة الطلب، وأي بيانات غير صحيحة قد تؤدي إلى رفض الطلب وفقدان الفترة المجانية.",
    ...
    phone: "رقم الجوال بصيغة دولية", phoneHelp: "سنتحقق من الرقم لأنه وسيلة إرسال روابط التدريب والتواصل معك عبر واتساب أو تيليجرام.", country: "الدولة",
    ...
    emailOtp: "رمز البريد", phoneOtp: "رمز الجوال", consent: "أوافق على استلام رسائل المستثمر الذكي الخدمية والتدريبية والتسويقية عبر البريد ووسائل التواصل المسجلة.",
    submit: "إنشاء الحساب", verifyEmail: "تأكيد البريد", sendPhone: "إرسال رمز الجوال", finish: "إرسال الطلب للمراجعة", loading: "جارٍ المتابعة…",
    ...
    noPlatform: "لا توجد منصة تداول مفعلة لهذا السوق حالياً. تواصل مع الإدارة.", success: "تم إرسال طلبك للمالك. احتفظ برقم الطلب الظاهر في صفحة الحالة.",
  },
  en: {
    title: "Create account", emailTitle: "Verify email", phoneTitle: "Verify mobile", intro: "Accurate details for faster, safer access",
    identityNotice: "Enter your name exactly as shown on your national ID, residence permit, or passport. You can correct it before review; incorrect details may cause rejection and loss of free access.",
    ...
    phone: "Mobile number in international format", phoneHelp: "We verify this number because training links and account contact may be sent through WhatsApp or Telegram.", country: "Country",
    ...
    emailOtp: "Email code", phoneOtp: "Mobile code", consent: "I agree to receive Smart Investor service, training, and promotional messages through my registered email and contact channels.",
    submit: "Create account", verifyEmail: "Verify email", sendPhone: "Send mobile code", finish: "Submit for review", loading: "Please wait…",
    ...
    noPlatform: "No trading platform is currently active for this market. Contact support.", success: "Your request was sent to the owner. Keep the reference shown on the status page.",
  },
};
...
export default function Register() {
  ...
  const [form, setForm] = useState({ full_name: "", email: "", phone_e164: "", country_code: "SA", market_code: "SA_MAIN", trading_platform_id: "", password: "", confirm: "", email_otp: "", phone_code: "", consent: false });
  ...
  const available = catalog.platforms.filter((item) => item.active && item.supported_market_codes?.includes(form.market_code));
  const platform = available.find((item) => item.id === form.trading_platform_id);

  useEffect(() => {
    Promise.resolve(base44.functions.invoke("registrationCatalog", {})).then((response) => setCatalog(response?.data || { platforms: [] })).catch(() => setCatalog({ platforms: [] }));
  }, []);
  useEffect(() => {
    if (!available.some((item) => item.id === form.trading_platform_id)) setForm((current) => ({ ...current, trading_platform_id: available[0]?.id || "" }));
  ...
  async function verifyEmail(event) {
    ...
    try {
      ...
      await base44.functions.invoke("authRegistration", { action: "start_phone_verification", ...form, preferred_language: language, marketing_consent: form.consent, phone_channel: "sms" });
      setStage("phone");
    } catch (issue) { setError(apiError(issue, language === "ar" ? "تعذر تأكيد البريد أو إرسال رمز الجوال" : "Unable to verify email or send the mobile code")); }
    finally { setLoading(false); }
  }
  ...
  async function finish(event) {
    ...
    } catch (issue) { setError(apiError(issue, language === "ar" ? "تعذر إرسال الطلب" : "Unable to submit your request")); }
    finally { setLoading(false); }
  }

  const title = stage === "email" ? t.emailTitle : stage === "phone" ? t.phoneTitle : t.title;
  return <AuthLayout icon={stage === "account" ? UserPlus : ShieldCheck} title={title} subtitle={t.intro} footer={<span>{t.hasAccount} <SessionLink to="/login" className="font-bold text-sky-600 dark:text-sky-400">{t.login}</SessionLink></span>}>
    ...
    <div className="mb-4 flex gap-2" aria-label="Registration progress">{["account", "email", "phone"].map((item) => <span key={item} className={`h-1.5 flex-1 rounded-full ${item === stage || (["email", "phone"].includes(stage) && item === "account") || (stage === "phone" && item === "email") ? "bg-sky-400" : "bg-slate-200 dark:bg-slate-700"}`} />)}</div>
    {stage === "account" && <form onSubmit={submitAccount} className="space-y-3">
      <div className="rounded-xl border border-sky-400/30 bg-sky-400/10 p-3 text-sm leading-6"><b className="flex items-center gap-2"><CheckCircle2 size={16} />{t.identityNotice}</b><p className="mt-2 text-slate-600 dark:text-slate-300">{t.directEmail}</p></div>
      ...
      <Field label={t.phone} name="phone_e164" placeholder="+9665XXXXXXXX" value={form.phone_e164} onChange={change} autoComplete="tel" help={t.phoneHelp} />
      ...
      {platform?.referral_url && <a className="block text-sm font-bold text-sky-600 underline" href={platform.referral_url} target="_blank" rel="noreferrer">{t.referral}</a>}
    ...
    {stage === "phone" && <form onSubmit={finish} className="space-y-3"><p className="text-sm text-slate-500">{form.phone_e164}</p><Field label={t.phoneOtp} name="phone_code" inputMode="numeric" maxLength={10} value={form.phone_code} onChange={change} /><Submit loading={loading} label={t.finish} loadingLabel={t.loading} /></form>}
  </AuthLayout>;
}
