...
export default function ApplicationStatus() {
  ...
  return <AuthLayout icon={ShieldCheck} title={isArabic ? "حالة طلب الوصول" : "Access application status"} subtitle={isArabic ? "متابعة واضحة من التسجيل حتى تفعيل السوق" : "Clear progress from registration to market activation"}>
    ...
    {!data ? <p>{isArabic ? "جارٍ تحميل حالة الطلب…" : "Loading application status…"}</p> : !data.registered ? <SessionLink className="primary-button" to="/register">{isArabic ? "بدء التسجيل" : "Start registration"}</SessionLink> : <div className="space-y-4">
      ...
      {(data.applications || []).map((application) => { const Icon = statusIcon[application.status] || Clock3; return <div key={application.id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex items-center gap-2"><Icon size={18} /><b>{application.unique_reference}</b></div><p className="mt-2 text-sm">{application.market_code} · {application.status}</p>{application.decision_reason && <p className="mt-2 text-sm text-slate-500">{application.decision_reason}</p>}</div>; })}
