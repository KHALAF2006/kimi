...
export default function RolesAdmin() {
  const { refresh: refreshIdentity } = useAuthorization();
  ...
  async function run(payload, success) {
    try {
      ...
      await invokeAppFunction("adminRoles", payload);
  ...
  async function createRole(event) {
    ...
    await run({ action: "create", role: newRole, reason }, "أُنشئ الدور المخصص وسُجل التغيير.");
  ...
  return <>
    ...
    {state.status && <div className="mx-auto mb-4 max-w-[1800px] rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300">{state.status}</div>}
      ...
      <div className="space-y-5">
        ...
        <section className="grid gap-5 xl:grid-cols-2">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#0d192a]">
            ...
            <div className="mt-4 grid gap-3"><select className="form-input" value={selectedMemberId} onChange={(event) => selectMember(event.target.value)}><option value="">اختر عضواً</option>{state.data?.members?.map((member) => <option key={member.id} value={member.id}>{member.full_name || member.customer_number || member.customer_id}</option>)}</select>{selectedMemberId && <div className="space-y-2">{state.data?.roles?.map((role) => <label key={role.id} className="flex items-center gap-2 rounded-xl border border-slate-200 p-3 text-sm dark:border-slate-700"><input type="checkbox" checked={memberRoleIds.includes(role.id)} onChange={(event) => setMemberRoleIds((values) => event.target.checked ? [...values, role.id] : values.filter((id) => id !== role.id))} /><UserRoundCog size={15} />{role.name_ar}</label>)}</div>}<input className="form-input" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="سبب الإسناد" /><button type="button" className="primary-button" disabled={state.busy || !selectedMemberId || reason.trim().length < 3} onClick={() => run({ action: "assign", member_id: selectedMemberId, role_ids: memberRoleIds, reason }, "حُفظت أدوار العضو وأُلغيت جلساته القديمة.")}><Save size={15} />حفظ الإسناد</button></div>
          </section>
        </section>
      </div>
    </div>
