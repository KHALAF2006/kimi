...
const sections = {
  market: { icon: Database, ar: ["كل اشتراك في سوقه المستقل", "السوق السعودي وشركات عقود الخيارات الأمريكية في مساحتين منفصلتين، لكل منهما شركاته وشريطه وبحثه وشمـوعه."], en: ["Each subscription has an isolated market", "Saudi equities and U.S. optionable companies live in separate workspaces, each with its own catalog, ticker, search and candles."] },
    ...
    <main>
      <section className="hero-section">
        ...
        <div className="relative z-10 mx-auto grid max-w-[1500px] items-center gap-12 px-5 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-28">
          <div><span className="eyebrow"><Activity size={14} />{isArabic ? "المستثمر الذكي للأسواق المشتركة" : "Smart Investor · Subscribed Markets"}</span><h1>{isArabic ? <>قرار أوضح يبدأ من <em>بيانات أوضح.</em></> : <>Clearer decisions begin with <em>clearer data.</em></>}</h1><p>{isArabic ? "منصة عربية وإنجليزية تفصل كل سوق بحسب الاشتراك النشط، وتجمع الأسعار والشموع والمؤشرات ومعلومات الشركة والمتابعة والتنبيهات دون خلط بين الأسواق." : "A bilingual platform that isolates each subscribed market while combining quotes, candles, company information, indicators, watchlists and alerts."}</p><div className="mt-8 flex flex-wrap gap-3"><SessionLink to="/register" className="primary-button">{isArabic ? "ابدأ حسابك" : "Create account"}<Arrow size={17} /></SessionLink><SessionLink to="/login" className="secondary-button">{isArabic ? "لدي حساب" : "I have an account"}</SessionLink></div><div className="hero-trust">{[isArabic ? "بيانات سوق فعلية" : "Actual market data", isArabic ? "معلومات شركة متكاملة" : "Integrated company information", isArabic ? "صلاحيات خلفية" : "Backend authorization"].map((label) => <span key={label}><CheckCircle2 size={15} />{label}</span>)}</div></div>
