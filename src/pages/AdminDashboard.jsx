...
export default function AdminDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      { label: "مشاكل البيانات المفتوحة", value: operations.issues?.length || 0, icon: Database },
  ...
  return <>
    ...
    <div className="mx-auto max-w-[1800px] space-y-6 px-4 pb-10">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(({ label, value, icon: Icon }) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#0d192a]"><div className="flex items-center justify-between text-slate-400"><span className="text-sm">{label}</span><Icon size={18} /></div><b className="mt-4 block text-3xl font-black">{Number(value).toLocaleString("ar-SA")}</b></div>)}</section>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{navigation.filter((item) => can(item.permission)).map(({ to, label, description, icon: Icon }) => <Link key={to} to={to} className="group rounded-2xl border border-slate-200 bg-white p-6 transition hover:-translate-y-1 hover:border-amber-400 hover:shadow-xl dark:border-slate-800 dark:bg-[#0d192a]"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-400/15 text-amber-500"><Icon size={19} /></span><b>{label}</b></div><p className="mt-3 text-sm leading-7 text-slate-500 dark:text-slate-400">{description}</p></Link>)}</section>
