...
export default function AdminDashboard() {
  ...
  return <>
    ...
    <div className="mx-auto max-w-[1800px] space-y-6 px-4 pb-10">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{navigation.filter((item) => can(item.permission)).map(({ to, label, description, icon: Icon }) => <SessionLink key={to} to={to} className="group rounded-2xl border border-slate-200 bg-white p-6 transition hover:-translate-y-1 hover:border-amber-400 hover:shadow-xl dark:border-slate-800 dark:bg-[#0d192a]"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-400/15 text-amber-500"><Icon size={19} /></span><b>{label}</b></div><p className="mt-3 text-sm leading-7 text-slate-500 dark:text-slate-400">{description}</p></SessionLink>)}</section>
