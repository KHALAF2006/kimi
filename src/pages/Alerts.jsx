  ...
  return <div className="space-y-5">
    ...
    {state.loading ? <StatusPanel loading/> : state.rules.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{state.rules.map((rule) => {
      ...
      return <article key={rule.id} className="content-card p-4">
        <div className="flex items-start justify-between gap-3"><div><SessionLink to={`/company?symbol=${encodeURIComponent(rule.symbol)}`} className="text-xl font-black text-sky-700 hover:underline dark:text-sky-300">{rule.symbol} · {isArabic ? rule.instrument?.name_ar : rule.instrument?.name_en}</SessionLink><p className="mt-1 text-sm text-slate-500">{condition ? condition[language === "ar" ? 1 : 2] : rule.condition}</p><small className="text-slate-400">{isArabic ? rule.instrument?.sector_ar : rule.instrument?.sector_en}{rule.quote?.last_price ? ` · ${Number(rule.quote.last_price).toFixed(2)} ر.س` : ""}</small></div><span className={rule.enabled ? "status-badge status-success" : "status-badge"}>{rule.enabled ? <Bell size={13}/> : <BellOff size={13}/>}</span></div>
