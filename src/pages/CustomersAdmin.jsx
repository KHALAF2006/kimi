    ...
    <main className="mx-auto max-w-[1900px] space-y-4 px-4 pb-10">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(420px,.8fr)]">
        ...
        <aside className="self-start rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/5 xl:sticky xl:top-24 dark:border-slate-800 dark:bg-[#0d192a]">{!selected ? <div className="grid min-h-[520px] place-items-center text-center text-slate-400"><div><UsersRound size={42} className="mx-auto mb-4" /><p>{t.choose}</p></div></div> : <div className="space-y-6">
          ...
          {selected.registration_integrity && <section className={`rounded-2xl border p-4 ${selected.registration_integrity.complete ? "border-emerald-400/60 bg-emerald-400/10" : "border-rose-400/60 bg-rose-400/10"}`}><div className="flex items-center justify-between gap-3"><b className="flex items-center gap-2"><ShieldCheck size={16} />{t.integrity}</b><span className={selected.registration_integrity.complete ? "text-emerald-600" : "text-rose-600"}>{selected.registration_integrity.complete ? t.integrityOk : t.integrityFailed}</span></div><p className="mt-2 font-mono text-xs text-slate-500">{selected.registration_integrity.unique_reference || "—"}</p></section>}
