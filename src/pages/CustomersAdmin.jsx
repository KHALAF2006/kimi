...
export default function CustomersAdmin() {
  ...
  const t = isArabic ? {
    ...
    notes: "ملاحظات الإدارة", addNote: "إضافة ملاحظة", cancel: "إلغاء", confirm: "تأكيد الإجراء", reason: "سبب الإجراء", body: "نص الرسالة", subject: "عنوان الرسالة", noResults: "لا توجد نتائج مطابقة", full: "بيانات كاملة", masked: "بيانات مقنّعة", open: "فتح الملف",
  } : {
    ...
    notes: "Admin notes", addNote: "Add note", cancel: "Cancel", confirm: "Confirm action", reason: "Action reason", body: "Message body", subject: "Message title", noResults: "No matching customers", full: "Full data", masked: "Masked data", open: "Open profile",
  };
  ...
  return <>
    ...
    <main className="mx-auto max-w-[1900px] space-y-4 px-4 pb-10">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(420px,.8fr)]">
        ...
        <aside className="self-start rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/5 xl:sticky xl:top-24 dark:border-slate-800 dark:bg-[#0d192a]">{!selected ? <div className="grid min-h-[520px] place-items-center text-center text-slate-400"><div><UsersRound size={42} className="mx-auto mb-4" /><p>{t.choose}</p></div></div> : <div className="space-y-6">
          <header className="border-b border-slate-200 pb-4 dark:border-slate-800"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-black">{selected.customer.full_name}</h2><p className="mt-1 text-xs text-slate-500">{selected.customer.customer_number} · {selected.customer.account_status}</p></div><button type="button" className="icon-button" onClick={() => setSelected(null)} aria-label={t.cancel}><X size={16} /></button></div><div className="mt-3 flex flex-wrap gap-3 text-xs"><span className="flex items-center gap-1"><CalendarDays size={14} />{dateLabel(selected.customer.created_date, language)}</span><span className="flex items-center gap-1"><Smartphone size={14} />{activeSessions} {t.activeSessions}</span></div><div className="mt-3 flex flex-wrap gap-2"><a className="secondary-button" href={`mailto:${selected.customer.email_normalized}`}><Mail size={14} />{selected.customer.email_normalized}</a>{digits(selected.customer.phone_e164) && <a className="secondary-button text-emerald-600" href={`https://wa.me/${digits(selected.customer.phone_e164)}`} target="_blank" rel="noopener noreferrer"><MessageCircle size={14} />{selected.customer.phone_e164}<ExternalLink size={11} /></a>}</div></header>
          <section><h3 className="mb-3 text-sm font-black">{t.markets}</h3><div className="grid grid-cols-2 gap-2">{Object.entries(MARKET_META).map(([code, meta]) => <div key={code} className={`rounded-2xl border p-3 ${marketStatuses[code] === "active" ? "border-emerald-400 bg-emerald-400/10" : "border-slate-200 dark:border-slate-700"}`}><div className="flex items-center justify-between"><span>{meta.mark}</span><small className={marketStatuses[code] === "active" ? "text-emerald-600" : "text-slate-400"}>{marketStatuses[code] || t.notActive}</small></div><b className="mt-2 block text-sm">{isArabic ? meta.ar : meta.en}</b></div>)}</div></section>
          <section><h3 className="mb-3 text-center text-xs font-black tracking-[.18em] text-slate-500">{t.period}</h3><div className="grid grid-cols-3 gap-3 rounded-2xl border border-sky-300/50 bg-sky-400/5 p-4 text-sm"><div><small className="text-slate-500">{t.start}</small><b className="mt-1 block text-emerald-500">{dateLabel(subscription?.starts_at, language)}</b></div><div><small className="text-slate-500">{t.end}</small><b className="mt-1 block">{dateLabel(subscription?.ends_at, language)}</b></div><div><small className="text-slate-500">{t.status}</small><b className="mt-1 block text-sky-500">{subscription?.status || t.noSubscription}</b></div></div></section>
    ...
    {action && <div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/70 p-4" role="dialog" aria-modal="true"><section className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#0d192a]"><div className="flex items-center justify-between"><h2 className="text-lg font-black">{action.kind === "message" ? t.message : action.kind === "add_note" ? t.addNote : t.confirm}</h2><button type="button" className="icon-button" onClick={() => setAction(null)}><X size={16} /></button></div><div className="mt-5 grid gap-3">{action.kind === "message" && <><input className="form-input" value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder={t.subject} /><select className="form-input" value={form.priority} onChange={(event) => setForm((current) => ({ ...current, priority: event.target.value }))}><option value="normal">Normal</option><option value="important">Important</option></select></>}{["message", "add_note"].includes(action.kind) && <textarea className="form-input min-h-28" value={form.message} onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} placeholder={t.body} />}<textarea className="form-input min-h-20" value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} placeholder={t.reason} /><div className="flex justify-end gap-2"><button type="button" className="secondary-button" onClick={() => setAction(null)}>{t.cancel}</button><button type="button" className="primary-button" disabled={state.busy || form.reason.trim().length < 3 || (["message", "add_note"].includes(action.kind) && form.message.trim().length < 3)} onClick={runAction}>{state.busy ? "…" : t.confirm}</button></div></div></section></div>}
    {state.error && <div className="fixed bottom-4 end-4 z-[140] max-w-sm rounded-xl border border-red-500/30 bg-red-950 p-4 text-sm text-red-200">{state.error}</div>}
    {state.status && <div className="fixed bottom-4 start-4 z-[140] max-w-sm rounded-xl border border-emerald-500/30 bg-emerald-950 p-4 text-sm text-emerald-200" role="status">{state.status}</div>}
  </>;
}
