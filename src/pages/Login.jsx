...
export default function Login() {
  ...
  return <AuthLayout icon={LogIn} title="تسجيل الدخول" subtitle={challenge?'أدخل الرمز المرسل إلى بريدك':'الوصول الآمن إلى حسابك'} footer={<span>ليس لديك حساب؟ <Link to="/register" className="text-emerald-400">إنشاء حساب</Link></span>}>
    {error&&<div className="mb-4 rounded-lg bg-red-500/10 p-3 text-sm text-red-300">{error}</div>}
    <form onSubmit={submit} className="space-y-4">{!challenge?<><Field label="البريد الإلكتروني" name="email" type="email" value={form.email} onChange={change}/><Field label="كلمة المرور" name="password" type="password" value={form.password} onChange={change}/><label className="flex items-center gap-2 text-sm text-slate-400"><input name="remember_me" type="checkbox" checked={form.remember_me} onChange={change}/>تذكرني على هذا الجهاز</label><Link to="/forgot-password" className="block text-sm text-emerald-400">نسيت كلمة المرور؟</Link></>:<Field label="رمز التحقق" name="otp" inputMode="numeric" maxLength={6} value={form.otp} onChange={change}/>}<Button className="h-12 w-full bg-emerald-400 text-slate-950 hover:bg-emerald-300" disabled={loading}>{loading&&<Loader2 className="ml-2 h-4 w-4 animate-spin"/>}{challenge?'تأكيد الدخول':'متابعة'}</Button></form>
  </AuthLayout>;
}
function Field({label,...props}){return <div className="space-y-2"><Label htmlFor={props.name}>{label}</Label><Input id={props.name} required className="h-12 border-white/10 bg-white/5 text-white" {...props}/></div>}