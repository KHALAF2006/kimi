...
export default function NotificationCenter() {
  ...
  return <>
    ...
    {state.open && <div className="fixed end-3 top-16 z-[100] w-[min(92vw,390px)] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl dark:border-slate-700 dark:bg-[#0d192a]">
      ...
      <div className="mt-3 max-h-[60vh] space-y-2 overflow-y-auto">{state.messages.filter((item) => !item.hidden_at).map((item) => <article key={item.id} className={`rounded-xl border p-3 text-sm ${item.priority === "critical" ? "border-red-400/50 bg-red-500/5" : "border-slate-200 dark:border-slate-700"}`}><b>{isArabic ? item.title_ar : item.title_en}</b><p className="mt-1 leading-6 text-slate-500">{isArabic ? item.body_ar : item.body_en}</p><div className="mt-2 flex gap-2"><button className="text-xs font-bold text-sky-600" onClick={() => action("mark_read", item)}>{isArabic ? "تمت القراءة" : "Mark read"}</button>{item.priority !== "critical" && <button className="text-xs text-slate-400" onClick={() => action("hide", item)}>{isArabic ? "إخفاء" : "Hide"}</button>}</div></article>)}{!state.messages.filter((item) => !item.hidden_at).length && <p className="py-8 text-center text-sm text-slate-400">{isArabic ? "لا توجد رسائل جديدة" : "No new messages"}</p>}</div>
    </div>}
    {feed.map((item) => <div key={item.id} className="fixed bottom-4 end-4 z-[90] max-w-sm rounded-2xl border border-sky-400/40 bg-white p-4 shadow-2xl dark:bg-[#0d192a]"><button className="absolute end-2 top-2 text-slate-400" onClick={() => action("hide", item)}><X size={15} /></button><b className="pe-6 text-sm">{isArabic ? item.title_ar : item.title_en}</b><p className="mt-1 text-sm text-slate-500">{isArabic ? item.body_ar : item.body_en}</p></div>)}
  </>;
}
