...
export default function NotificationCenter() {
  ...
  const feed = useMemo(() => state.preferences.feed_enabled ? state.messages.filter((item) => item.message_type !== "support" && item.feed_eligible && !item.hidden_at && !item.read_at).slice(0, 1) : [], [state.messages, state.preferences.feed_enabled]);
  ...
  return <>
    <button className="icon-button relative" onClick={() => setState((current) => ({ ...current, open: !current.open }))} aria-label={isArabic ? "التنبيهات" : "Notifications"}><Bell size={17} />{state.unread > 0 && <span className="absolute -end-1 -top-1 min-w-4 rounded-full bg-red-500 px-1 text-[10px] text-white">{state.unread > 9 ? "9+" : state.unread}</span>}</button>
    {state.open && <div className="fixed end-3 top-16 z-[100] w-[min(92vw,390px)] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl dark:border-slate-700 dark:bg-[#0d192a]">
      <div className="flex items-center justify-between"><b className="flex items-center gap-2"><Inbox size={17} />{isArabic ? "التنبيهات" : "Notifications"}</b><button className="icon-button" onClick={() => setState((current) => ({ ...current, open: false }))}><X size={15} /></button></div>
      <SessionLink className="mt-3 flex w-full items-center justify-center rounded-xl bg-sky-500 px-3 py-2 text-sm font-black text-white" to="/messages" onClick={() => setState((current) => ({ ...current, open: false }))}>{isArabic ? "فتح مركز الرسائل" : "Open message center"}</SessionLink>
