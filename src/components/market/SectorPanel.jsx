...
export default function SectorPanel({ sector, marketCode = "SA_MAIN", onSelectCompany = () => {}, onResetWidth = () => {} }) {
  ...
  if (state.error || !state.data?.sector) return <section className="company-panel-empty text-red-600"><Info /><h2>{isArabic ? "تعذر تحميل بيانات القطاع" : "Sector data unavailable"}</h2><p>{isArabic ? "لم تكتمل قراءة شركات القطاع من قاعدة السوق. أعد المحاولة، أو ابحث عن سهم من القائمة." : "The sector constituents could not be read from the market database. Retry or select a stock."}</p><button type="button" className="secondary-button" onClick={() => setRetryKey((value) => value + 1)}>{isArabic ? "إعادة المحاولة" : "Retry"}</button></section>;
