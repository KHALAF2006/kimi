...
export default function CatalogSearch() {
  ...
  return <><DataNotice/><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالرمز أو الاسم أو القطاع" className="mb-5 h-12 border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900"/>{state.loading ? <p className="text-slate-400">جارٍ البحث…</p> : state.error ? <p className="text-red-600">{state.error}</p> : <><p className="mb-3 text-sm text-slate-500">{state.rows.length} نتيجة</p><MarketTable rows={state.rows}/></>}</>;
}
