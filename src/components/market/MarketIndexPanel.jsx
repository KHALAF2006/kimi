...
export default function MarketIndexPanel({ indexCode = "TASI", marketCode = "SA_MAIN", onResetWidth = () => {} }) {
  ...
  useEffect(() => {
    ...
    invokeAppFunction("marketRead", { symbol: indexCode, market_code: marketCode, instrument_code: indexCode })
      ...
      .catch((error) => active && setState({ loading: false, data: null, error: error?.response?.data?.code || error?.response?.data?.error || error?.message || "index_fetch_failed" }));
    return () => { active = false; };
  }, [indexCode, marketCode]);

  if (state.loading) return <section className="company-panel-empty"><Loader2 className="animate-spin" /><p>{isArabic ? "جارٍ تحميل مؤشر السوق…" : "Loading market index…"}</p></section>;
  if (state.error || !state.data?.instrument) return <section className="company-panel-empty text-red-600"><Info /><h2>{isArabic ? "تعذر تحميل مؤشر السوق" : "Market index unavailable"}</h2><p>{isArabic ? "لم تكتمل أرشفة شموع المؤشر بعد. حاول مجددًا بعد دورة المزامنة." : "The index candle archive is not ready yet. Retry after the next sync cycle."}</p></section>;
