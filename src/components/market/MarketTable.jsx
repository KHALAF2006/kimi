...
export default function MarketTable({ rows = [], marketCode = "", selectedSymbol = "", onSelect = null, detailsTimeframe = "" }) {
  ...
  return <div className="market-table-shell">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return <tr key={row.id || row.symbol} className={selectedSymbol === row.symbol ? "selected-market-row" : ""}>
              <td><SessionLink to={companyDashboardPath(row.symbol, detailsTimeframe, marketCode)} onClick={(event) => select(event, row)} className="market-symbol">{row.symbol}<ExternalLink size={11} /></SessionLink></td>
              <td><SessionLink to={companyDashboardPath(row.symbol, detailsTimeframe, marketCode)} onClick={(event) => select(event, row)} className="block"><b className="text-slate-950 dark:text-white">{isArabic ? row.name_ar : row.name_en}</b><span className="mt-1 block text-xs text-slate-500">{isArabic ? row.sector_ar : row.sector_en}</span>{row.screener_match?.label && <span className="mt-1 block text-[11px] font-black text-violet-700 dark:text-violet-300">{row.screener_match.label}</span>}{signalDate(row) && <span className="mt-1 block text-[11px] font-bold text-sky-700 dark:text-sky-300">{isArabic ? "الإشارة: " : "Signal: "}{signalDate(row)}</span>}<div className="mt-1"><LossFlagBadge flag={row.warning_flag || row.loss_classification?.level} compact /></div></SessionLink></td>
    ...
    <div className="divide-y divide-slate-200 dark:divide-slate-800 md:hidden">
      {rows.map((row) => {
        const quote = row.quote || {};
        const direction = quoteDirection(quote.change_percent);
        return <SessionLink key={row.id || row.symbol} to={companyDashboardPath(row.symbol, detailsTimeframe, marketCode)} onClick={(event) => select(event, row)} className={"block p-4 " + (selectedSymbol === row.symbol ? "bg-sky-50 dark:bg-sky-400/10" : "")}>
          <div className="flex items-start justify-between gap-3"><div><b>{row.symbol} · {isArabic ? row.name_ar : row.name_en}</b><p className="mt-1 text-xs text-slate-500">{isArabic ? row.sector_ar : row.sector_en}</p>{row.screener_match?.label && <p className="mt-1 text-[11px] font-black text-violet-700 dark:text-violet-300">{row.screener_match.label}</p>}{signalDate(row) && <p className="mt-1 text-[11px] font-bold text-sky-700 dark:text-sky-300">{isArabic ? "الإشارة: " : "Signal: "}{signalDate(row)}</p>}<div className="mt-2"><LossFlagBadge flag={row.warning_flag || row.loss_classification?.level} compact /></div></div><div className="text-left" dir="ltr"><b className="text-lg">{formatNumber(quote.last_price, language)}</b><p className={"text-sm font-black market-" + direction}>{Number(quote.change_percent || 0) > 0 ? "+" : ""}{formatNumber(quote.change_percent, language)}%</p><p className="mt-1 text-[10px] text-slate-500" dir={isArabic ? "rtl" : "ltr"}>{quoteStatusLabel(quote, isArabic)}</p></div></div>
        </SessionLink>;
      })}
    </div>
