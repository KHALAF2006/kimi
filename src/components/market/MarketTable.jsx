  ...
  return <div className="market-table-shell">
    ...
    <div className="divide-y divide-slate-200 dark:divide-slate-800 md:hidden">
      {rows.map((row) => {
        const quote = row.quote || {};
        const direction = quoteDirection(quote.change_percent);
        return <SessionLink key={row.id || row.symbol} to={companyDashboardPath(row.symbol, detailsTimeframe)} onClick={(event) => select(event, row)} className={"block p-4 " + (selectedSymbol === row.symbol ? "bg-sky-50 dark:bg-sky-400/10" : "")}>
