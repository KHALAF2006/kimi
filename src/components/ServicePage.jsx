...
export default function ServicePage({ title, description, functionName, payload={}, children = null }) {
  ...
  useEffect(()=>{
    ...
    if(Object.hasOwn(payload,"market_code")&&!Reflect.get(payload,"market_code"))return()=>{active=false};
    invokeAppFunction(functionName,payload)
      .then(data=>active&&setState({loading:false,data,error:""}))
      .catch(e=>active&&setState({loading:false,data:null,error:e.response?.data?.error||e.message}));
    return()=>{active=false};
