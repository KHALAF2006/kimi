...
export default function AuthLayout({ icon: Icon, title, subtitle = "", footer = null, children }) {
  ...
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="relative min-h-screen overflow-hidden bg-slate-50 px-4 py-8 text-slate-950 dark:bg-slate-950 dark:text-white">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(14,165,233,.14),transparent_32%),radial-gradient(circle_at_80%_85%,rgba(15,23,42,.08),transparent_35%)] dark:bg-[radial-gradient(circle_at_20%_10%,rgba(14,165,233,.12),transparent_32%)]" />
      <header className="relative mx-auto flex max-w-6xl items-center justify-between">
        <SessionLink to="/" className="flex items-center gap-3 font-black"><span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-sm text-sky-400 dark:bg-sky-400 dark:text-slate-950">SI</span><span>المستثمر الذكي</span></SessionLink>
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-88px)] w-full max-w-md items-center">
        ...
        <div className="mb-8 text-center">
          <div className="mb-4 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-sky-400 shadow-lg shadow-sky-500/20">
