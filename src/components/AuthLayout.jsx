...
export default function AuthLayout({ icon: Icon, title, subtitle = "", footer = null, children }) {
  const { isArabic, language, setLanguage, theme, toggleTheme } = usePreferences();
  return (
    <div dir={isArabic ? "rtl" : "ltr"} className="relative min-h-screen overflow-hidden bg-slate-50 px-4 py-8 text-slate-950 dark:bg-slate-950 dark:text-white">
      ...
      <header className="relative mx-auto flex max-w-6xl items-center justify-between">
        <Link to="/" className="flex items-center gap-3 font-black"><span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-sm text-amber-400 dark:bg-amber-400 dark:text-slate-950">KMY</span><span>{isArabic ? "منصة كيمي" : "KMY Platform"}</span></Link>
        <div className="flex gap-2"><button type="button" className="icon-button" onClick={() => setLanguage(language === "ar" ? "en" : "ar")} aria-label="language"><Languages size={17} /></button><button type="button" className="icon-button" onClick={toggleTheme} aria-label="theme">{theme === "dark" ? <Sun size={17} /> : <Moon size={17} />}</button></div>
      </header>
