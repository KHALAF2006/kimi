# Smart Investor Multi-Market Platform — المستثمر الذكي

منصة عربية/إنجليزية متعددة الأسواق. تدعم السوق الرئيسية السعودية، وسوقًا مستقلًا من 110 شركات أمريكية ذات عقود خيارات، وسوقًا مستقلًا من 33 مؤشرًا وصندوقًا أمريكيًا، مع اشتراك منفصل لكل سوق وعزل كامل للسوق النشط. هذا المستودع هو تنفيذ Full-stack لـBase44، وليس واجهة عرض منفصلة.

أصبح لكل قطاع ملف كامل يعرض مؤشره المرجح وشارت الشموع ومناطق المستثمر وشركاته. ويجمع البحث المحمي الأسهم ومؤشرات القطاعات ومؤشر السوق العام `TASI` في دليل أدوات واحد يدعم الاقتراح أثناء الكتابة ولوحة المفاتيح. ويدعم الشارت سحب محوري السعر والزمن، تكبير الفأرة، فرشاة ومنحنى انسيابيين، حافظة رسم دائمة، وعمليات إخفاء وإظهار ومسح جماعية محفوظة في الخلفية.

## مصدر الشركات وبيانات السوق

- المصدر المثبت لكتالوج أسماء الشركات والقطاعات والمرجع اليومي التاريخي: تقرير تداول السعودية التفصيلي ليوم `2026-07-21`.
- الملف الرسمي المحفوظ: `base44/data/official-main-market-catalog-2026-07-21.json`.
- العدد: `270` شركة، و`270` رمزًا فريدًا.
- بصمة SHA-256: `4BCC19FD271E1D84D1390E8B2E311046243A8CC9B79B024FF43850C4D8F31337`.
- هذا الملف ليس موجز أسعار حياً، ولا تُعرض قيمه على أنها محدثة.
- المصدر العام الافتراضي يجلب شموع `15m`، ويحسب الإغلاق السابق من آخر شمعة في جلسة التداول السابقة بدلاً من بيانات `meta` غير الموثوقة.
- تعرض واجهة العميل وقت اللقطة والاستلام والتغطية فقط. ويمكن تبديل محول المصدر دون تغيير واجهات القراءة بوضع `SMART_INVESTOR_MARKET_DATA_MODE=licensed`.
- تقوم `marketIngestion` بعمل upsert لكتالوج الشركة، ثم تحفظ كل قراءة في `QuoteObservation` قبل ترقية اللقطة السليمة إلى `QuoteLatest` و`CandleChunk`.
- تُحفظ هوية كل أداة بنوع صريح: `equity` أو `sector_index` أو `market_index`. مؤشر `TASI` له سجل مستقل ومرادفات بحث، وتُستورد شموعه اليومية التاريخية مرة واحدة من رمز المؤشر `^TASI.SR` إلى `CandleChunk` ثم تُقرأ محليًا مثل بقية الشموع.
- مؤشر القطاع الداخلي له رمز ثابت من شكل `SECTOR:<CODE>`، وتُبنى شموعه من الشموع المحفوظة لشركات القطاع بقراءة مجمعة واحدة، لا بطلب قاعدة بيانات منفصل لكل شركة.
- ألوان القطاعات تعرض النسبة نصيًا وتراعي الثيم. الأحمر محجوز لانخفاض يتجاوز `1.5%` في جلستين متتاليتين؛ أما الانخفاض غير المؤكد فيستخدم حالة محايدة غير حمراء.

### مزامنة السوق السعودي T+15

مسار السعر الوحيد:

```text
Base44 automation
→ قفل دورة التحديث
→ جلب متوازٍ للمصدر العام أو طلب مجمع من محول المزود
→ توحيد الرموز
→ فحوص الوقت وOHLC والتغير والتغطية
→ QuoteObservation
→ QuoteLatest + CandleChunk
→ marketRead
→ الجدول والرسم والتنبيهات
```

- يبدأ النشر بتأخير 15 دقيقة، ويحسب الخادم التغير من `previous_close`.
- تغطية `99%+` سليمة، ومن `95%` إلى أقل من `99%` جزئية، وأقل من `95%` تفشل دون ترقية اللقطة.
- السهم المفقود يحتفظ بآخر قيمة سليمة بعلامة `stale`; لا يُستبدل بسعر ثابت أو تقديري.
- الجدول والرسم والتنبيهات يقرأون قاعدة البيانات فقط. فتح الرسم لا يتصل بمزود خارجي.
- يستخدم هذا التطبيق نظام Base44 Workflows؛ لذلك تبقى ملفات الوظائف بلا `automations` قديمة، وتوجد جداول السوقين داخل `base44/workflows/*.jsonc`. يجب التحقق من ظهور كل Workflow وسجل تشغيل ناجح في لوحة التطبيق قبل تفعيل أي سوق للعملاء.
- يحسب كل سوق الجلسة والتأخير والفواصل بتوقيته: `Asia/Riyadh` للسعودي و`America/New_York` للأمريكي، ويمنع الخادم القراءة أو البحث أو التنبيه في سوق لا يملكه الاشتراك.
- التشغيل المتوقع بعد التفعيل: 23 استدعاءً في يوم التداول، أو نحو 506 أرصدة أتمتة في شهر من 22 جلسة.

## التشغيل المحلي

```bash
npm install
npm test
npm run typecheck
npm run lint
npm run build
npm run dev
```

لربط الواجهة محليًا بالـbackend المرجعي الحقيقي:

```bash
VITE_SMART_INVESTOR_REFERENCE_API=/reference-api npm run dev
```

يمر المسار `/reference-api` عبر Vite إلى `http://127.0.0.1:3001/api` لتجنب أخطاء CORS. هذا المسار محلي فقط؛ الإنتاج يستخدم وظائف Base44 المحمية.

## Base44

المصدر الوحيد لوظائف الخلفية هو:

`base44/functions/<name>/entry.ts`

وتوضع الوحدات المشتركة داخل:

`base44/shared/`

يتحقق الأمر التالي من أن جميع الوظائف قابلة للتجميع دون إنشاء نسخ متفرقة:

```bash
npm run build:app-editor-functions
```

يتحقق `npm test` أن النسخ المولدة مطابقة للمصدر وليست قديمة.

المستودع هو مصدر الحقيقة للكود. أما مخططات الكيانات فيجب مزامنتها إلى بيئة اختبار أولاً، ثم إلى الإنتاج بعد نسخة استرجاع ومراجعة عدد السجلات.

بوابة الربط المحلية:

```bash
npm run base44:check-link
```

إذا كان الهدف مشروع Base44 Backend مستقلًا موجودًا، يسجل المالك الدخول ثم يربط المجلد ويشغّل النشر الكامل:

```bash
npx base44 login
npx base44 link
npm run base44:check-link
npx base44 deploy -y
```

لا يُحذف أي مخطط مكرر أو سجل قبل نسخة الاسترجاع وموافقة المالك الصريحة وقت التنفيذ.

قبل النشر يجب أن تنجح الاختبارات، ثم تُزامن الموارد التي يتحقق من عددها `npm test` إلى بيئة الاختبار. السوق الأمريكي يحفظ الشموع التاريخية مرة واحدة، ويحدث معلومات الشركات بدفعات دوارة من مصادر SEC وNasdaq ومحول إجراءات الشركات. تحققت مطابقة CIK لجميع رموز الكتالوج الأمريكي `110/110` مع دليل SEC الرسمي.

## أسرار بيانات السوق

تُضاف في أسرار Base44 فقط، ولا تدخل GitHub أو الواجهة:

- `SMART_INVESTOR_MARKET_DATA_PROVIDER_CODE`
- `SMART_INVESTOR_MARKET_DATA_URL`
- `SMART_INVESTOR_MARKET_DATA_TOKEN`
- `SMART_INVESTOR_MARKET_DATA_MODE`

القيمة الافتراضية القديمة المتوافقة لـ`SMART_INVESTOR_MARKET_DATA_MODE` هي `experimental_public` ولا تحتاج رابطاً أو رمزاً. عند اختيار `licensed` يجب أن يعيد الرابط لقطة مجمعة لكل السوق مع `provider_as_of`, و`quotes[]` و—إن كانت متاحة—شموع `15m`.

## قنوات الإرسال المركزية

- لا يملك المستخدم أي صفحة أو وظيفة لإضافة قناة تنبيه. إدارة القنوات محصورة بالمالك في `/admin/delivery` وتُرفض أيضاً من الخلفية لغيره.
- لكل سجل قناة رمز سوق صريح. محرك التنبيهات يطابق سوق القاعدة والأداة والسعر وحدث التسليم والقناة قبل أي اتصال خارجي.
- تيليجرام وواتساب يقبلان أسماء أسرار يحددها المالك لكل قناة، مثل `TELEGRAM_SA_MAIN_TOKEN` أو `WHATSAPP_US_OPTIONS_TOKEN`. تُضاف القيم في أسرار Base44 فقط ولا تدخل Git أو الواجهة.
- قناة واتساب تحفظ معرّف رقم Meta وإصدار Graph API واسم القالب المعتمد ولغته. قالب التنبيه يرتب: اسم الشركة، الرمز، السعر، الشرط، ووقت السعر.
- البريد الجماعي مستقل عن مركز رسائل العملاء. يُنشئ المالك حملة لأسواق محددة، وتبني الخلفية لقطة مستلمين من العملاء النشطين ذوي الاشتراك الساري والموافقة المسجلة، ثم ترسل دفعات قابلة للاستئناف دون خلط الأسواق.
- يعالج `deliveryWorker` أحداث التسليم وحملات البريد بدفعات محدودة في Workflow كل 15 دقيقة خلال ساعات التشغيل، ويحتفظ بمفاتيح منع التكرار وحالات النجاح والفشل.
- خدمة Base44 المدمجة للبريد تستهلك رصيد تكامل لكل مستلم؛ لذلك تعرض صفحة المالك العدد والتكلفة التقديرية قبل إنشاء الحملة.

## الأمان

- القراءة والكتابة المباشرة من المتصفح مرفوضة في جميع كيانات Base44.
- الوصول يتم عبر وظائف خلفية تتحقق من المستخدم والجلسة النشطة والاشتراك والدور وملكية الكائن.
- دخول جهاز جديد يبطل الجلسة السابقة بعد نجاح التحقق.
- أحداث التسليم لها مفتاح منع تكرار وحد أقصى للمحاولات، وقنواتها المركزية لا ترتبط بحساب المستخدم.
- لا يوجد fallback صامت: المصدر العام اختيار صريح، وعند فشله تبقى آخر لقطة سليمة مع وقتها الحقيقي ولا تُستبدل ببيانات الكتالوج.
- الأدوار المخصصة والصلاحيات والاشتراكات تُفحص في الخادم، مع منع افتراضي وسجل قبل/بعد ورقم مراجعة.
- تشغيل جلب السوق اليدوي يتطلب صلاحية مستقلة وجلسة نشطة.
- هوية الأداة المالية مركبة من السوق ورمز الأداة، والأسواق غير المرخصة تبقى معطلة.

## التحقق

`npm test` يتحقق من عدد الشركات، تفرد الرموز، بصمة الملف، نافذة T+15، جدولة السوق، حساب التغير من إغلاق جلسة التداول السابقة، رفض OHLC والنسب المتناقضة، التخزين قبل الترقية، رفض الوصول المباشر لجميع الكيانات، هندسة القنوات والقياسات، وحواجز الصلاحيات والملكية والمراجعات المتزامنة.

توثيق تكامل GitHub مع Base44: <https://docs.base44.com/Integrations/Using-GitHub>

### Company intelligence feed

The `companyIntelligence` backend function is fail-closed and never fabricates announcements, major-shareholder ownership, or financial statements. Configure these Base44 secrets before enabling its schedules:

- `SAUDI_EXCHANGE_COMPANY_FEED_URL`: the licensed/authenticated batch endpoint.
- `SAUDI_EXCHANGE_COMPANY_FEED_TOKEN`: the endpoint bearer token.

The endpoint must return official `https://*.saudiexchange.sa/` provenance URLs for every accepted record. `CompanyIntelligenceDaily` runs after close from Sunday through Thursday; `CompanyFinancialsTwiceWeekly` runs on Monday and Thursday.
