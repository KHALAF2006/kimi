...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const source = sourceRows[0] ? await base44.asServiceRole.entities.DataSource.update(sourceRows[0].id, sourceData) : await base44.asServiceRole.entities.DataSource.create({ code: "US_OPTIONS_CANONICAL_PROJECTION", ...sourceData });
    const instruments = rows(await base44.asServiceRole.entities.Instrument.filter({ market_code: US_OPTIONS_MARKET_CODE })).filter((item) => US_OPTIONS_SYMBOLS.has(item.symbol) && item.status !== "delisted");
    ...
    run = await base44.asServiceRole.entities.IngestionRun.create({
      ...
      status: "running",
      source_id: source.id,
      notes: "U.S. optionable daily, weekly, monthly and technical signal projection"
    });
    ...
    const existingSnapshots = rows(await base44.asServiceRole.entities.IndicatorSnapshot.list("-source_as_of", 5e3)).filter((item) => instrumentIds.has(item.instrument_id));
    ...
    await base44.asServiceRole.entities.IngestionRun.update(run.id, {
      ...
      success_count: instruments.length - failed,
      failed_count: failed,
      coverage_percent: (instruments.length - failed) / instruments.length * 100,
      snapshot_version: slotKey,
      notes: `daily:${projectedDaily.length};higher:${higherChunks.length};signals:${snapshots.length};skipped:${failed}`
    });
