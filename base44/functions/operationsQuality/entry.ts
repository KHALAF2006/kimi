// GENERATED from operationsQuality/source.ts. Do not edit directly.

// base44/functions/operationsQuality/source.ts
...
Deno.serve(async (req) => {
  try {
    ...
    const [sources, openIssues, runs, deliveryEvents] = await Promise.all([
      ...
      base44.asServiceRole.entities.IngestionRun.list("-started_at", 1e3),
    ...
    if (action === "reconcile_recovered_issues") {
      ...
      if (batch.length) await base44.asServiceRole.entities.DataQualityIssue.bulkUpdate(batch.map((issue) => ({
        id: issue.id,
        status: "resolved",
    ...
    return Response.json({
      sources,
      issues,
      runs: requestedMarket ? runs.filter((run) => run.market_code === requestedMarket) : runs,
      issue_summary: {
        active_count: issues.length,
        recovered_pending_reconciliation: recovered.length,
        stored_open_count: requestedMarket ? openIssues.filter((issue) => issueMarket(issue, sourcesById) === requestedMarket).length : openIssues.length,
