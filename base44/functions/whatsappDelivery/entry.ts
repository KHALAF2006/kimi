  ...
  try {
    ...
    for (const recipient of allowed) {
      const response = await fetch(`https://graph.facebook.com/${graphVersion}/${phoneNumberId}/messages`, {
        ...
        body: JSON.stringify({
          messaging_product: 'whatsapp', to: recipient.phone_e164, type: 'template',
          template: {
    ...
    const updated = await base44.asServiceRole.entities.DeliveryEvent.update(event.id, {
      status: 'sent', attempt_count: Number(event.attempt_count || 0) + 1, provider_code: `sent:${messageIds.length}`, delivered_at: new Date().toISOString(),
    });
