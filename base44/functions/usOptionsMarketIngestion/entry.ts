  ...
  try {
    ...
    for (const item of output) {
      ...
      chunks.push({
        ...
        symbol: item.symbol,
        interval: "15m",
        chunk_key: `${US_OPTIONS_MARKET_CODE}:${item.symbol}:15m:${item.sessionDate}`,
        session_date: item.sessionDate,
        start_time: item.bars[0].time,
        end_time: item.bars.at(-1).time,
        bars: item.bars,
        bar_count: item.bars.length,
        checksum: await checksum(item.bars),
        source_id: source.id,
        run_id: run.id,
        snapshot_version: snapshotVersion,
        provider_as_of: item.providerAsOf,
        received_time: received,
        quality_status: freshnessStatus === "fresh" ? "verified" : "stale",
        canonical_version: "us-options-intraday-v1",
        is_final: isFinal,
        bucket_count: item.bars.length,
