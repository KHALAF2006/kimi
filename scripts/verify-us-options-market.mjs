...
assert.match(ingestion, /queuePersonalAlertMessage/, "U.S. options customer alerts must use the customer-scoped in-app message path");
assert.doesNotMatch(ingestion, /DeliveryEvent\.(create|bulkCreate)/, "U.S. options customer rules must never enter the owner broadcast queue");
assert.doesNotMatch(ingestion, /DeliveryChannel\.filter/, "U.S. options customer rules must not depend on centralized owner channels");
